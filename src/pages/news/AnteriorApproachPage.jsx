    ...
    <>
      ...
      <header className="bg-gradient-to-r from-blue-600 to-teal-500 text-white py-20">
        <div className="container mx-auto text-center px-6">
          ...
          >
            <Button
              size="lg"
              className="bg-white text-blue-600 hover:text-teal-600"
              asChild
            >
      ...
      <main className="container mx-auto px-6 py-16 space-y-24">
        ...
        <section aria-labelledby="definition-heading" className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <motion.div initial="hidden" animate="visible" custom={1} variants={sectionVariants} className="max-w-xl">
          ...
          <motion.div initial="hidden" animate="visible" custom={2} variants={sectionVariants} className="flex justify-center">
            <img
        ...
        {/* Medical Tourism */}
        <section aria-labelledby="tourism-heading" className="bg-gradient-to-r from-green-50 to-green-100 p-12 rounded-2xl">
          <div className="max-w-2xl mx-auto mb-10 text-center">
            <h2 id="tourism-heading" className="inline-flex items-center justify-center text-3xl font-extrabold text-green-800">
        ...
        {/* Journey Steps */}
        <motion.section initial="hidden" animate="visible" custom={4} variants={sectionVariants} className="mt-16">
          <h2 className="text-3xl font-bold text-center mb-8">{content.journey.title}</h2>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 items-start">
            {content.journey.steps.map((step, i) => {
              const isRecovery = step.id === 'recovery';
              const lines = isRecovery ? step.description.split('\n').filter((l) => l.trim()) : [];
              ...
              return (
                <motion.div
                  ...
                  custom={i + 1}
                  variants={sectionVariants}
                  className={`flex flex-col h-full bg-white rounded-2xl shadow-lg p-6 ${isRecovery ? 'lg:col-span-3' : ''}`}
                >
                  <div className="text-blue-600 text-xl font-semibold mb-4">{step.step}</div>
        ...
        <aside className="bg-gradient-to-r from-blue-600 to-teal-500 text-white rounded-2xl p-12 text-center">
          ...
          <Button size="lg" className="bg-white text-blue-600 hover:text-teal-500" asChild>
            <Link to={`${home()}#contact`} onClick={(e) => handleAnchorClick(e, '#contact')}>
              {content.cta.button}
            </Link>
          </Button>
        ...
        <nav className="text-center">
          <Button asChild variant="link">
            <Link className="inline-flex items-center text-gray-600 hover:text-gray-900" to={go('news')}>
              ...
              {t('safeTreatmentAbroad.backToNews')}
            </Link>
          </Button>
        </nav>
      </main>
