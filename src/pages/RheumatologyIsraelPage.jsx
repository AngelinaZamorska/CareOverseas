import React, { useEffect } from 'react';
import { Helmet } from 'react-helmet';
import { motion } from 'framer-motion';
import {
  Bone,
  Dna,
  UserCheck,
  Microscope,
  CheckCircle,
  ArrowRight
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useTranslation } from 'react-i18next';
import { useNavigate, useLocation } from 'react-router-dom';

const RheumatologyIsraelPage = () => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const location = useLocation();

  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  const scrollToContact = () => {
    const el = document.getElementById('contact');
    if (el) el.scrollIntoView({ behavior: 'smooth' });
  };

  const handleContactClick = () => {
    if (location.pathname !== '/') {
      navigate('/');
      setTimeout(scrollToContact, 200);
    } else {
      scrollToContact();
    }
  };

  const conditions = [
    t('rheumatologyIsraelPage.condition1'),
    t('rheumatologyIsraelPage.condition2'),
    t('rheumatologyIsraelPage.condition3'),
    t('rheumatologyIsraelPage.condition4'),
    t('rheumatologyIsraelPage.condition5'),
    t('rheumatologyIsraelPage.condition6')
  ];

  return (
    <div className="text-base leading-relaxed">
      <Helmet>
        <title>{t('rheumatologyIsraelPage.title')}</title>
        <meta name="description" content={t('rheumatologyIsraelPage.description')} />
      </Helmet>

      {/* Hero Section */}
      <section className="relative bg-cyan-50 py-20 md:py-28 lg:py-32">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.h1
            className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-cyan-700 mb-4"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
          >
            {t('rheumatologyIsraelPage.header')}
          </motion.h1>
          <motion.p
            className="text-md sm:text-lg md:text-xl text-gray-600 max-w-3xl mx-auto mb-8"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.3 }}
          >
            {t('rheumatologyIsraelPage.subtitle')}
          </motion.p>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} transition={{ delay: 0.5 }}>
            <Button
              size="lg"
              className="bg-cyan-600 hover:bg-cyan-700 px-6 md:px-8 py-3 md:py-4 text-base md:text-lg font-semibold"
              onClick={handleContactClick}
            >
              {t('rheumatologyIsraelPage.getQuote')} <ArrowRight className="ml-2 h-5 w-5 inline" />
            </Button>
          </motion.div>
        </div>
      </section>

      {/* Why Israel Section */}
      <section className="py-16 md:py-20 lg:py-24 bg-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6">
            {t('rheumatologyIsraelPage.whyIsraelTitle')}
          </h2>
          <p className="text-center text-gray-600 max-w-2xl mx-auto mb-12">
            {t('rheumatologyIsraelPage.whyIsraelSubtitle')}
          </p>
          <div className="grid gap-6 sm:gap-8 md:gap-10 md:grid-cols-2 lg:grid-cols-4">
            {[UserCheck, Dna, Microscope, Bone].map((Icon, i) => (
              <motion.div
                key={i}
                className="p-6 bg-gray-50 rounded-xl shadow hover:shadow-md transition-shadow"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.15 }}
              >
                <Icon className="h-8 w-8 text-cyan-600 mb-4" />
                <h3 className="text-lg font-semibold mb-2">
                  {t(`rheumatologyIsraelPage.feature${i + 1}Title`)}
                </h3>
                <p className="text-gray-600 text-sm sm:text-base">
                  {t(`rheumatologyIsraelPage.feature${i + 1}Desc`)}
                </p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Conditions Section */}
      <section className="py-16 md:py-20 lg:py-24 bg-cyan-50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-10">
            {t('rheumatologyIsraelPage.conditionsTitle')}
          </h2>
          <div className="grid lg:grid-cols-2 gap-10">
            <div className="space-y-4">
              {conditions.map((cond, idx) => (
                <motion.div
                  key={idx}
                  className="flex items-start text-gray-800"
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  viewport={{ once: true }}
                  transition={{ delay: idx * 0.1 }}
                >
                  <CheckCircle className="h-5 w-5 text-cyan-600 mt-1 mr-2 flex-shrink-0" />
                  <span className="text-base sm:text-lg">{cond}</span>
                </motion.div>
              ))}
            </div>
            <motion.div
              className="rounded-lg overflow-hidden shadow-md max-w-md mx-auto"
              initial={{ opacity: 0, x: 20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
            >
              <img
                src="/joint-pain.jpg"
                alt="Joint pain"
                className="w-full h-auto object-cover"
              />
            </motion.div>
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-16 md:py-20 lg:py-24 bg-gradient-to-r from-cyan-600 to-sky-600 text-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold mb-4">
            {t('rheumatologyIsraelPage.ctaTitle')}
          </h2>
          <p className="max-w-2xl mx-auto mb-8 text-base sm:text-lg">
            {t('rheumatologyIsraelPage.ctaSubtitle')}
          </p>
          <Button
            size="lg"
            className="bg-white text-cyan-600 px-6 md:px-10 py-3 md:py-4 font-bold text-base md:text-lg"
            onClick={handleContactClick}
          >
            {t('rheumatologyIsraelPage.ctaButton')}
          </Button>
        </div>
      </section>
    </div>
  );
};

export default RheumatologyIsraelPage;